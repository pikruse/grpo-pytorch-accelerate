...
def gen_samples(inputs):
    ...
    for i, inp in enumerate(inputs):
        ...
        for a in answers[i*num_pre_Q:(i+1)*num_pre_Q]: # answers are grouped by input, so we need to group them by input here. 8 answers per input
            ...
            rewards.append(math_reward_correct(inp, a) + reward_format(inp, a))
...
def GRPO_step(batch):
    ...
    Args:
        batch (dict): A dictionary containing the following keys:
            ...
            - 'inputs' (torch.Tensor): Input tensor with shape (batch_size, sequence_length).
            - 'rewards' (torch.Tensor): Reward tensor with shape (batch_size, num_pre_Q).
            - 'refs' (torch.Tensor): Reference log probability tensor with shape (batch_size, sequence_length).
    ...
    def get_per_token_logps(logits, input_ids):
        ...
        Args:
            logits (torch.Tensor): Model output logits with shape (batch_size, sequence_length, vocab_size).
            input_ids (torch.Tensor): Input token IDs with shape (batch_size, sequence_length).
        
        Returns:
            torch.Tensor: Log probabilities for each token with shape (batch_size, sequence_length - 1).
        """

        # Exclude the last logit because it corresponds to the prediction for the next token,
        # resulting in shape (batch_size, sequence_length - 1, vocab_size)
        logits = logits[:, :-1, :]  # (B, L-1, V)

        # Exclude the first input ID since we don't have corresponding logits,
        # resulting in shape (batch_size, sequence_length - 1)
        input_ids = input_ids[:, 1:]  # (B, L-1)

        # Initialize a list to store log probabilities for each token
        per_token_logps = []
        for logits_row, input_ids_row in zip(logits, input_ids):
            
            # Compute log softmax, resulting in shape (sequence_length - 1, vocab_size)
            log_probs = logits_row.log_softmax(dim=-1)
            
            # Gather the log probability corresponding to each token, shape (sequence_length - 1)
            token_log_prob = torch.gather(log_probs, dim=1, index=input_ids_row.unsqueeze(1)).squeeze(1)
            
            # Append the token log probabilities to the list
            per_token_logps.append(token_log_prob)

        # Convert the list to a tensor with shape (batch_size, sequence_length - 1)
